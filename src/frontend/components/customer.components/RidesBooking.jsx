...
function RidesBooking () {
  ...
  const [allRides, setAllRides] = useState([]) // Renamed to allRides for clarity
  const [filteredRides, setFilteredRides] = useState([]) // State to hold filtered rides
  ...
  useEffect(() => {
    axiosApi
      ...
      .then(response => {
        ...
        setFilteredRides(response.data) // Initialize filteredRides with all rides
      })
      .catch(error => {
        console.log(error)
      })
  ...
  const applyFilters = () => {
    const filtered = allRides.filter(ride => {
      const heightRestriction = parseFloat(ride.height_restriction)
      const meetsHeightCriteria = heightRestriction >= (minHeight || 0)
      const meetsBrokenCriteria = showBroken || !ride.broken
      const meetsRainedOutCriteria = showRainedOut || !ride.rained_out

      return (
        meetsHeightCriteria && meetsBrokenCriteria && meetsRainedOutCriteria
      )
    })
    ...
    <Container>
      ...
      {showFilters && (
        <Filters>
          ...
          >
            <label>Show Working Rides</label>
            <input
              type='checkbox'
              checked={showBroken}
          ...
          >
            <label>Show Non-Rained Out Rides</label>
            <input
              type='checkbox'
      ...
      <h4 className='TitleForType'>Rides:</h4>
